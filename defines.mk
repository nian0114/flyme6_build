...
rm -rf ~/.local/share/apktool/framework/[0-9]*-$(APKTOOL_BOARD_TAG).apk;\
...
rm -rf ~/.local/share/apktool/framework/[0-9]*-$(APKTOOL_VENDOR_TAG).apk;\
...
rm -rf ~/.local/share/apktool/framework/[0-9]*-$(APKTOOL_MERGED_TAG).apk;\
$(INSTALL_FRAMEWORKS) $(1) $(APKTOOL_MERGED_TAG); \
cp ~/.local/share/apktool/framework/1-$(APKTOOL_MERGED_TAG).apk $(APKTOOL_FRAME_PATH_BOARD_MODIFY)/1-$(APKTOOL_BOARD_TAG).apk
endef
...
`ls ~/.local/share/apktool/framework/[0-9]*-$(APKTOOL_BOARD_TAG).apk | sed 's/^/-I /g'`
endef

# used for aapt to merged resouce
define get_vendor_installed_framework_params
`ls ~/.local/share/apktool/framework/[0-9]*-$(APKTOOL_VENDOR_TAG).apk | sed 's/^/-I /g'`
endef

# used for aapt to merged resouce
define get_merged_installed_framework_params
`ls ~/.local/share/apktool/framework/[0-9]*-$(APKTOOL_MERGED_TAG).apk | sed 's/^/-I /g'`
endef
...
# get the apk in ~/.local/share/apktool/framework/ which the install framework resouce apks stored
define get_include_aapt_res
`ls ~/.local/share/apktool/framework/[0-$(1)]*-$(APKTOOL_VENDOR_TAG).apk | sed 's/^/-I /g'`
endef
